...
export function MyJamiyasSection({ jamiyas }: { jamiyas: DashboardJamiya[] }) {
  return (
    <section>
      <SectionHeader
        title="My circles"
        ...
        action={
          <Button asChild size="sm" variant="outline">
            <Link href={'/jamiyas' as Route}>View all</Link>
          </Button>
        }
      />
      ...
      {jamiyas.length === 0 ? (
        <EmptyState
          ...
          actionHref={'/jamiyas/new' as Route}
        />
      ) : (
        <ul className="divide-y divide-border rounded-xl border border-border bg-card">
          {jamiyas.map((item) => (
            <li key={item.membershipId}>
              <Link
                href={`/jamiyas/${item.jamiya.slug}` as Route}
