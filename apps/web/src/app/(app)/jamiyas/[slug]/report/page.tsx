...
import { PrintReportButton } from '@/features/jamiya/components/print-report-button';
...
export default async function CirclePrintReportPage({ params }: Props) {
  ...
  if (!user) redirect(`/login?next=/jamiyas/${slug}/report`);
    ...
    <main className="mx-auto max-w-3xl px-6 py-10 print:px-0 print:py-0">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 print:hidden">
        <Button asChild variant="outline" size="sm">
          <Link href={`/jamiyas/${slug}` as Route}>Back to circle</Link>
        </Button>
        <PrintReportButton />
      </div>
