...
async function decideGraceFormAction(formData: FormData) {
  ...
  revalidatePath(`/jamiyas/${slug}/officer`);
  revalidatePath(`/jamiyas/${slug}/community`);
}
...
export default async function OfficerConsolePage({ params }: Props) {
  ...
  if (!user) redirect(`/login?next=/jamiyas/${slug}/officer`);
    ...
    <div className="mx-auto max-w-5xl space-y-8 px-6 py-10">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            href={`/jamiyas/${slug}/report` as Route}
            className="rounded-md border border-border px-3 py-1.5 text-sm"
          >
            Report
          </Link>
