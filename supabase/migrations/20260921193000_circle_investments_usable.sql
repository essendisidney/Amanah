-- Phase 9: Usable circle investments + statement visibility.

CREATE OR REPLACE FUNCTION public.update_circle_investment(
...
DECLARE
  v_uid UUID := auth.uid();
  v_inv public.circle_investments%ROWTYPE;
BEGIN
  IF v_uid IS NULL THEN
    RETURN jsonb_build_object('ok', false, 'error', 'UNAUTHENTICATED');
  END IF;

  SELECT * INTO v_inv FROM public.circle_investments WHERE id = p_investment_id;
  IF NOT FOUND THEN
  ...
  IF p_status IS NOT NULL AND p_status NOT IN ('planned', 'active', 'closed') THEN
    RETURN jsonb_build_object('ok', false, 'error', 'INVALID_STATUS');
  END IF;
  IF p_current_value IS NOT NULL AND p_current_value < 0 THEN
  ...
  SET
    ...
    name = CASE
      WHEN p_name IS NOT NULL AND char_length(btrim(p_name)) BETWEEN 2 AND 120
        THEN btrim(p_name)
      ELSE name
    END,
    closed_on = CASE
      WHEN COALESCE(nullif(btrim(p_status), ''), status) = 'closed'
        THEN COALESCE(closed_on, CURRENT_DATE)
      ELSE closed_on
    END,
    updated_at = NOW()
  ...
  VALUES (
    v_uid, 'update', 'circle_investment', p_investment_id,
    jsonb_build_object(
  ...
  RETURN jsonb_build_object('ok', true, 'investment_id', p_investment_id);
END;
$$;

REVOKE ALL ON FUNCTION public.update_circle_investment(UUID, NUMERIC, TEXT, TEXT, TEXT) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.update_circle_investment(UUID, NUMERIC, TEXT, TEXT, TEXT) TO authenticated;

-- Append circle projects to member statements (circle-level, not per-member P&L).
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(jsonb_build_object(
    ...
    'notes', i.notes,
    'recorded_by', COALESCE(p.full_name, p.email, 'Officer')
  ) ORDER BY i.started_on DESC NULLS LAST, i.created_at DESC), '[]'::jsonb)
  INTO v_investments
  FROM public.circle_investments i
  LEFT JOIN public.profiles p ON p.id = i.created_by
  WHERE i.jamiya_id = p_jamiya_id
    AND i.status IN ('planned', 'active');

  SELECT COALESCE(sum(i.current_value), 0) INTO v_invest_value
  FROM public.circle_investments i
  WHERE i.jamiya_id = p_jamiya_id AND i.status IN ('planned', 'active');
  ...
  RETURN jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'circle_investments_value', v_invest_value
    ),
    ...
    'circle_investments', v_investments
  );
END;
$$;
