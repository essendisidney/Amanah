-- Include circle slug + registration href on circle KYC review notifications.
...
BEGIN
  ...
  SELECT name, slug INTO v_circle_name, v_circle_slug FROM public.jamiyas WHERE id = v_doc.jamiya_id;
  ...
  IF p_status IN ('approved', 'rejected') AND v_doc.uploaded_by IS NOT NULL THEN
    ...
    VALUES (
      ...
      jsonb_build_object(
        ...
        'status', p_status,
        'href', CASE
          WHEN v_circle_slug IS NULL THEN '/circles'
          ELSE '/circles/' || v_circle_slug || '/registration'
        END
      )
    );
  END IF;
