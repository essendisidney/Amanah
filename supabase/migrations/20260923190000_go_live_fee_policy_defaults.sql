-- Go-live fee policy: circles stay opt-in; Sadaka fee 0 until board endorses.

ALTER TABLE public.charity_campaigns
  ALTER COLUMN fee_bps SET DEFAULT 0;

-- New campaigns start at 0 bps (donation_addon). Ops raises after endorsement.
UPDATE public.charity_campaigns
SET fee_bps = 0, updated_at = NOW()
WHERE sharia_board_endorsed IS NOT TRUE
  AND fee_bps > 0;
...
CREATE OR REPLACE FUNCTION public.submit_sadaka_campaign(
  ...
  p_public_media_urls text[] DEFAULT NULL::text[]
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
AS $function$
DECLARE
  v_uid UUID := auth.uid(); v_slug TEXT; v_base TEXT; v_n INT := 1; v_id UUID;
  v_cat public.sadaka_category; v_media TEXT[] := COALESCE(p_public_media_urls, '{}'::text[]);
BEGIN
  IF v_uid IS NULL THEN RETURN jsonb_build_object('ok', false, 'error', 'UNAUTHENTICATED'); END IF;
  IF NOT EXISTS (SELECT 1 FROM public.members m WHERE m.user_id = v_uid AND m.status = 'active') THEN
  ...
  END IF;
  IF btrim(COALESCE(p_beneficiary_name, '')) = '' OR btrim(COALESCE(p_beneficiary_phone, '')) = '' OR btrim(COALESCE(p_beneficiary_kyc_doc_url, '')) = '' THEN
    RETURN jsonb_build_object('ok', false, 'error', 'BENEFICIARY_KYC_REQUIRED');
  END IF;
  IF p_category NOT IN ('medical', 'funeral', 'education', 'business_startup', 'emergency_disaster', 'institutional') THEN
  ...
  INSERT INTO public.charity_campaigns (
    ...
    v_slug, btrim(p_title), left(btrim(p_story), 280), btrim(p_story), p_target_amount, 'KES',
    'pending_review', false, 'donation_addon', 0, v_uid, v_cat,
    btrim(p_beneficiary_name), btrim(p_beneficiary_phone), btrim(p_beneficiary_kyc_doc_url),
    nullif(btrim(COALESCE(p_cover_image_url, '')), ''), v_media
  ) RETURNING id INTO v_id;
  INSERT INTO public.notifications (user_id, type, channel, title, body, data)
  SELECT p.id, 'system', 'in_app', 'Sadaka campaign pending review',
    'A new campaign awaits approval: ' || btrim(p_title),
    jsonb_build_object('campaign_id', v_id, 'href', '/admin/sadaka')
  FROM public.profiles p WHERE p.platform_role IN ('platform_admin', 'super_admin', 'compliance_officer');
