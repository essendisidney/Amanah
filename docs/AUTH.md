...
5. Phone OTP (Kenya): set `TAIFA_API_KEY` + `TAIFA_SENDER_ID` (e.g. `SIDNET`) on **Vercel**. Apply `otp_codes` migration. Supabase Phone/Twilio **not** required. Use `SMS_BYPASS=true` only in local dev. Product reminder SMS uses those same Vercel Taifa keys via `/api/cron/dispatch` (`dispatchSmsOutboxViaTaifa`).
